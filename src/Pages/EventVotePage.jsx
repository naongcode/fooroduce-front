...
export default function EventVotePage() {
  ...
  const { eventResult, votedTruckIds, setVotedTruckIds, fetchVoteResult } = useEvent();
  ...
  const itemsPerPage = 3; // 한 페이지에 보여줄 트럭 수

 
  const trucksWithVoteCount = useMemo(() => {
    return (eventData?.trucks || []).map((truck) => {
      const match = eventResult.find((result) => truck.truckId === result.truckId);
      return {
        ...truck,
        voteCount: match?.voteCount || 0,
      };
    });
  }, [eventData?.trucks, eventResult]);
  ...
  // 카테고리 변경 시 현재 페이지를 1로 초기화 
  useEffect(() => {
    setCurrentPage(1)
  }, [activeCategory])

  // 행사 정보 및 투표 종료 여부 가져오기
  useEffect(() => {
    const fetchEventData = async () => {
      try {
        
        const res = await axiosInstance.get(
          `/events/${eventId}?page=${currentPage - 1}&size=${itemsPerPage}&menuType=${activeCategory}`,
        );
        ...
        const hasVotingEnded = currentDate > voteEndDate;
        setIsVotingEnded(hasVotingEnded);

        // 투표가 종료되었으면 바로 결과 모달 표시
        if (hasVotingEnded) {
          setShowPodium(true);
        }
      } catch (err) {
  ...
  }, [eventId, currentPage, activeCategory, itemsPerPage]);
  ...
  const allCategories = ['전체', '한식', '양식', '일식', '멕시칸', '분식', '디저트', '기타'];

 
  const displayTrucks = trucksWithVoteCount;
    ...
    <div>
      ...
      {!isVotingEnded ? (
        <>
          ...
          {displayTrucks.length === 0 && <TruckNotFound />}
        ...
        <div className="text-center py-20 text-2xl text-gray-700 font-semibold">
          투표가 종료되어 푸드트럭 목록을 더 이상 볼 수 없습니다.
        </div>
      )}
...
const TruckCard = ({ truck, handleVote, isVoted, isVotingEnded, activeCategory }) => {
  ...
  const displayMenu = useMemo(() => {
    if (!truck.menus || truck.menus.length === 0) {
      return null; // 메뉴가 없으면 표시할 것이 없음
    }

    // '전체' 카테고리이거나, 현재 활성 카테고리와 일치하는 메뉴가 없는 경우 첫 번째 메뉴를 기본으로 사용
    if (activeCategory === '전체') {
      return truck.menus[0];
    }

    // 현재 activeCategory와 일치하는 메뉴를 찾음 (첫 번째 일치하는 메뉴 반환)
    const matchingMenu = truck.menus.find((menu) => menu.menuType === activeCategory);

    // 일치하는 메뉴가 있다면 그 메뉴를 사용, 없으면 첫 번째 메뉴를 사용
    return matchingMenu || truck.menus[0];
  }, [truck.menus, activeCategory]); 
      ...
      <div className="h-56 overflow-hidden relative group">
        ...
        <div className="absolute top-4 right-4">
          <span className="bg-white/90 backdrop-blur-sm text-indigo-800 text-xs font-bold px-3 py-1.5 rounded-full shadow-md">
            {displayMenu.menuType ?? '기타'} 
          </span>
        </div>
      </div>

      <div className="p-6">
        <h3 className="text-2xl font-bold text-gray-800 mb-3">{truck.truckName}</h3>
        ...
        <div className="mb-6">
          ...
          <div
            className={`overflow-hidden transition-all duration-300 ${
              expandedMenus.includes(truck.truckId) ? 'max-h-[500px] mt-4' : 'max-h-0'
            }`}
          >
                ...
                >
                  <div className="flex items-center">
                    <div className="w-16 h-16 bg-indigo-100 rounded-lg overflow-hidden mr-4">
                      <img src={item.menuImage} alt={'메뉴 이미지'} className="w-full h-full object-cover" />
                    </div>
                    <div className="flex-1">
                      <h4 className="font-bold text-gray-800">{item.menuName}</h4>
                      <p className="text-indigo-600 font-medium">{item.menuPrice}</p>
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-lg transition-all duration-300 font-medium whitespace-nowrap cursor-pointer ${
              isVoted || isVotingEnded 
                ? 'bg-gray-200 text-gray-600'
                : 'bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white shadow-md hover:shadow-lg'
            }`}
            disabled={isVoted || isVotingEnded} 
          >
            {isVotingEnded ? (
              <>투표 종료</>
            ) : isVoted ? (
...
const MenuDetail = ({ selectedTruck, setSelectTruck, votedTruckIds, handleVote, isVotingEnded }) => {
  const ref = useRef();
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div ref={ref} className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-2xl font-bold text-indigo-800">{selectedTruck.truckName}</h3>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              />
              <div className="mt-4">
                <p className="text-gray-600 mb-4">{selectedTruck.description}</p>
                <div className="flex items-center">
                  <span className="bg-indigo-100 text-indigo-800 px-3 py-1 rounded-full text-sm font-medium">
                    {selectedTruck.category ?? '한식'}
                  </span>
                  <span className="ml-4 text-indigo-600 font-medium">
                    <i className="fas fa-thumbs-up mr-1"></i> {selectedTruck.voteCount} 투표
                  </span>
                </div>
              </div>
            </div>
                    ...
                    <div className="flex flex-col md:flex-row">
                      ...
                      <div className="md:w-2/3 md:pl-6">
                        <div className="flex justify-between items-start mb-2">
                          <h5 className="text-xl font-bold text-gray-800">{menu.menuName}</h5>
                          <span className="text-indigo-700 font-bold">{menu.menuPrice}원</span>
                        </div>
              ...
              <div className="mt-8 flex justify-end">
                <button
                  ...
                  className={`px-6 py-3 rounded-lg transition-all duration-300 font-medium whitespace-nowrap !rounded-button cursor-pointer ${
                    votedTruckIds.includes(selectedTruck.truckId) || isVotingEnded // 투표 종료 시 및 이미 투표 시 비활성화
                      ? 'bg-gray-200 text-gray-600'
                      : 'bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white shadow-md hover:shadow-lg'
                  }`}
                  disabled={votedTruckIds.includes(selectedTruck.truckId) || isVotingEnded} // 투표 종료 시 및 이미 투표 시 비활성화
                >
                  {isVotingEnded ? (
                    <>투표 종료</>
